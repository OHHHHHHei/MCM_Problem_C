...
- **阶段二：概率性拯救 (基于技术分)**
    ...
    其中 $\sigma(z) = \frac{1}{1+e^{-z}}$。参数 $\beta_{judge}$ 控制拯救决策的**陡峭程度**：$\beta \to 0$ 时评审随机选择，$\beta \to \infty$ 时几乎必救技术分较高者。
    
> **敏感性分析**: 为验证结论稳健性，将测试 $\beta \in \{1, 2, 4\}$ 以检验关键结论是否保持一致。

**尺度与饱和性 (Scale & Saturation)**:
- $J$ 使用**归一化评审分**：$J_{i,t} = \text{RawScore}_{i,t} / \max_j \text{RawScore}_{j,t}$，范围 $[0,1]$
- $\Delta J$ 典型范围：$[-0.3, 0.3]$；当 $|\Delta J| > 2/\beta$ 时 Save 近似确定性，当 $|\Delta J| < 1/\beta$ 时处于"悬念区"
...
全局一致率：对所有周次取平均（或按淘汰人数加权）
$$\text{Agreement} = \frac{1}{T} \sum_{t=1}^{T} P_t(E_{rank} = E_{pct})$$

**差异分类**：在差异案例中，分别统计 Rank/Percentage 规则救回高人气选手的概率。
...
2. **规则内秉背离度 (Intrinsic Rule Divergence)**:
    ...
    $$P(E_{RuleA} \neq E_{RuleB}) = \sum_{k=1}^K w_t^{(k)} \cdot \mathbb{I}\left( \text{Sim}(R_A, \pi^{(k)}) \neq \text{Sim}(R_B, \pi^{(k)}) \right)$$
...
3. **封顶假设 (The Capping Hypothesis)**:
   - **Rank Rule** 对超级巨星是 **Anti-Populist** 的：将无限的粉丝热情压缩为 "Rank 1"，无法抵消 Rank Last 的评审劣势。
   - **Percentage Rule** 对超级巨星是 **Pro-Fan** 的：允许海量粉丝票淹没评审分劣势，使其根本无法跌入 Bottom 2。
   - **可检验指标**：
     - Save 触发率: $\text{Act}(R) = \Pr(\text{Target} \in \text{Bottom2})$
     - Save 边际效果: $\Delta P(\text{Flip}) = P(\text{Flip} | R + \text{Save}) - P(\text{Flip} | R)$
   - **结论**：在 Pct 下 Act 极低（如 <5%），故 Save 大多不触发；只有 Rank + Save 才能触发熔断机制。
